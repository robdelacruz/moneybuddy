...
{:else}
    ...
    {#if mode == ""}
        ...
        <div class="mb-2">
            <input class="input w-full" name="accountname" id="accountname" type="text" placeholder="Account Name" bind:value={frm_name}>
        </div>
        {:else}
        <div class="flexrow mb-2">
            <div class="mr-2 w-1/2">
                <input class="input w-full" name="accountname" id="accountname" type="text" placeholder="Stock Name" bind:value={frm_name}>
            </div>
            <div class="w-1/2">
                <input class="input w-full" name="unitprice" id="unitprice" type="text" placeholder="Unit Price" bind:value={frm_unitprice} on:blur="{e => frm_unitprice = formatnum(frm_unitprice)}">
            </div>
        </div>
        {/if}
        <div class="flexrow mb-2">
            <div class="mr-2 w-1/2">
                <select class="input w-full" id="accounttype" name="accounttype" placeholder="Account Type" bind:value={frm_accounttype}>
                    <option value={data.BANKACCOUNT}>Bank Account</option>
                    <option value={data.STOCKACCOUNT}>Stock</option>
                </select>
            </div>
            <div class="w-1/2">
                <select class="input w-full" id="currency" name="currency" placeholder="Currency" bind:value={frm_currencyid}>
        ...
        <div class="mb-2">
            <input class="input w-full" name="ref" id="ref" type="text" placeholder="Reference No" bind:value={frm_ref}>
        </div>
        <div class="mb-2">
        <textarea class="input w-full" name="memo" id="memo" placeholder="Memo" rows="4" bind:value={frm_memo}></textarea>
        </div>
        ...
        <div class="flexrow justify-between">
            ...
            <div class="flexrow">
                {#if account.accountid != 0}
                <button class="btn bg-input mr-2" on:click|preventDefault={onMove}>Move...</button>
                {/if}
    ...
    {:else if mode == "move"}
        <div class="">
            ...
            <select class="input mb-2" id="movebook" name="movebook" bind:value={frm_movebookid}>
                {#each root.books as b}
                <option value={b.bookid}>{b.name}</option>
                {/each}
            </select>
            <div>
                <button class="btn bg-inputok mr-2" on:click|preventDefault={onConfirmMove}>Move</button>
                <a href="/" class="action" on:click|preventDefault={onConfirmCancel}>Cancel</a>
            </div>
        </div>
    {:else if mode == "delete"}
        <div class="">
